...
class CompanyIdentityVerificationController extends Controller
{
    ...
    public function statusCompanyIdentifyVerification()
    {
        ...
        if (!$identifyVerification) {
            ...
            return response()->json(['status' => null, 'success' => 'No files have been sent for company identity verification.']);
        } elseif ($identifyVerification->status === 1) {
            //  your account is already verified
            return response()->json(['status' => 1, 'success' => 'Identity cannot be verified more than once - your account is already verified']);
        } else {
            // The documentation files have been sent -> Not replay Verify -> Witting Replay Verify
            return response()->json(['status' => 0, 'success' => 'The documentation files have been sent and a response will be received within 3 to 5 business days.']);
        }
    }
}
